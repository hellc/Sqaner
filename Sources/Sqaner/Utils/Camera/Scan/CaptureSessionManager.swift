...
/// The CaptureSessionManager is responsible for setting up and managing the
    ...
    private func processRectangle(rectangle: Quadrilateral?, imageSize: CGSize) {
        if let rectangle = rectangle {
            ...
            self.rectangleFunnel.add(
                ...
                strongSelf.displayRectangleResult(
                    rectangleResult: RectangleDetectorResult(rectangle: rectangle, imageSize: imageSize)
                )
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                if strongSelf.noRectangleCount > strongSelf.noRectangleThreshold {
                    // Reset the currentAutoScanPassCount, so the threshold
                    // is restarted the next time a rectangle is found
    ...
    func photoOutput(_ captureOutput: AVCapturePhotoOutput,
        ...
        if let sampleBuffer = photoSampleBuffer,
            let imageData = AVCapturePhotoOutput.jpegPhotoDataRepresentation(
                forJPEGSampleBuffer: sampleBuffer, previewPhotoSampleBuffer: nil
            ) {
