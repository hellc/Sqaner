...
/// Feed rectangles to a `RectangleFeaturesFunnel` instance, and it will call the completion
///  block with a rectangle whose confidence is high enough to be displayed.
final class RectangleFeaturesFunnel {
    ...
    private final class RectangleMatch: NSObject {
        ...
        /// A higher score indicates that many recently added
        /// rectangles are very close to the rectangle of this instance.
        var matchingScore = 0
    ...
    /// Determines the maximum size of `rectangles`. Increasing this value will impact performance.
    ...
    /// The minumum number of matching rectangles (within the `rectangle` queue),
    /// to be confident enough to display a rectangle.
    ...
    /// 5. If the best rectangle is different than the currently displayed rectangle,
    ///  informs the listener that a new rectangle should be displayed
    ///     5a. The currentAutoScanPassCount is incremented every time a new rectangle is displayed.
    ///     If it passes the autoScanThreshold, we tell the listener to scan the document.
    /// - Parameters:
    ...
    /// Otherwise if the second passed rectangle (rect2) is close
    /// to the currently displayed rectangle, we pick this one.
    /// Finally, if none of the passed in rectangles are close to the currently displayed rectangle,
    ...
    private func updateRectangleMatches() {
        ...
        for (index1, currentRect) in rectangles.enumerated() {
            for (index2, rect) in rectangles.enumerated() {
                if index2 > index1 && currentRect.matches(rect.rectangleFeature, withThreshold: matchingThreshold) {
