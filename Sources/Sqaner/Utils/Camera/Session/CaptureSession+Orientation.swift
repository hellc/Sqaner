...
extension CaptureSession {
    ...
    func setImageOrientation() {
        ...
        motion.startAccelerometerUpdates(to: OperationQueue()) { data, error in
            ...
            /// If the device is reporting a specific landscape orientation,
            /// we'll use it over the accelerometer's update.
            /// We don't use this to check for "portrait"
            /// because only the accelerometer works when portrait lock is enabled.
