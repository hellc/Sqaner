...
final class ZoomGestureController {
    ...
    @objc func handle(pan: UIGestureRecognizer) {
        ...
        guard let zoomedImage = self.image.scaledImage(atPoint: scaledDraggedCornerViewCenter, scaleFactor: 2.5, targetSize: self.quadView.bounds.size) else {
            return
        }
...
final class CropController: UIViewController {
    ...
    private lazy var nextButton: UIBarButtonItem = {
        let button = UIBarButtonItem(title: "Готово", style: .done, target: self, action: #selector(self.onDoneButtonTap))
        button.tintColor = navigationController?.navigationBar.tintColor
        return button
    }()
    
    private lazy var cancelButton: UIBarButtonItem = {
        let button = UIBarButtonItem(title: "Отмена", style: .plain, target: self, action: #selector(self.onCancelButtonTap))
        button.tintColor = navigationController?.navigationBar.tintColor
        return button
    }()
    ...
    override public func viewDidLoad() {
        ...
        self.toolbarItems = [cancelButton, UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: self, action: nil), nextButton]
